...
from db.repository.blog import create_new_blog, retrieve_blog, retrieve_all_blogs, update_blog_by_id, delete_blog_by_id
...
async def get_blog(id: int, db: Session = Depends(get_db)):
    ...
    if not blog:
        raise HTTPException(
            detail=f'blog with {id} does not exist', status_code=status.HTTP_404_NOT_FOUND)
    return blog
